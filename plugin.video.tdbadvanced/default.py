...
addon_id = 'plugin.video.tdbadvanced'
ADDON = xbmcaddon.Addon(id=addon_id)
AddonID='plugin.video.tdbadvanced'
AddonTitle="[COLOR orange]TDB [/COLOR] [COLOR white]Advanced Settings[/COLOR]"
...
PATH = "TDB Advanced Settings"            
BASEURL = "http://test.com"
H = 'http://'
skin         =  xbmc.getSkinDir()
EXCLUDES     = ['backupdir','plugin.video.tdbadvanced','script.module.addon.common','repository.tdbegley.official','backup','backup.zip']
...
zip = 'special://home/addons/plugin.video.tdbadvanced'
urlbase      =  'None'
...
USB          =  xbmc.translatePath(os.path.join('special://home/addons/plugin.video.tdbadvanced',''))
...
def BACKUP():  
    ...
    if choice == 0:
	 return
      ...
      exclude_dirs_full =  ['backupdir','plugin.video.tbqizard','repository.schismtv.addons','script.skinshortcuts','plugin.program.super.favourites']
...
def INDEX():
	...
	#addDir('[COLOR red][B]STEP 1 - WIPE[/B][/COLOR]','url',6,ART+'freshstart.png',FANART,'')
	addDir('[COLOR lime][B]INSTALL[/B][/COLOR]',BASEURL,20,ART+'install.png',FANART,'')
...
def WipeXBMC():
    if skin!= "skin.confluence":
        dialog.ok('[COLOR=blue][B]TDB[/B][/COLOR][COLOR=green][/COLOR] Custom Builds Tool','Please switch to the default Confluence skin','before performing a wipe.','')
        ...
        if choice == 1:
            ...
            exclude_dirs_full =  ['backupdir','plugin.video.tdbadvanced','repository.tdbegley.official']
            exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
    ...
    if choice2 == 0:
        ...
        dp.create("[COLOR=blue][B]TDB[/B][/COLOR] Custom Builds Tool","Wiping Install",'', 'Please Wait')
        try:
    ...
    REMOVE_EMPTY_FOLDERS()
    REMOVE_EMPTY_FOLDERS()
    dialog.ok('[COLOR=blue][B]TDB [/B][/COLOR] Custom Builds Tool','Wipe Successful, please restart XBMC/Kodi for changes to take effect.','','')
