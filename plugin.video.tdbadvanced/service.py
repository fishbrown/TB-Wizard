...
CHECKVERSION  =  os.path.join(USERDATA,'advancedversion.txt')
			...
			if vernumber > 0:
				...
				for newversion,fresh in match:
					if newversion > vernumber:
						...
						if choice == 1: 
							if fresh =='false': # TRUE
								updateurl = "https://archive.org/download/tdbadvanced/update_wiz.txt"
								...
								for url in match:
									...
									dp.create("[COLOR=orange][B]Thomas Begley[/B][/COLOR][COLOR=white] Advanced[/COLOR]","Downloading ",'', 'Please Wait')
									...
									dialog = xbmcgui.Dialog()
									dialog.ok("[COLOR=orange][B]Thomas Begley[/B][/COLOR][COLOR=white] Advanced[/COLOR]", "To save changes you now need to force close Kodi, Press OK to force close Kodi")
									
									Common.KillKodi()
									
							else:
								dialog.ok('[COLOR=orange][B]Thomas Begley[/B][/COLOR][COLOR=white] Advanced[/COLOR]','A FRESH START is required for the update... Run the FRESH START in the NEXT WINDOW then INSTALL the new build version','','')
								xbmc.executebuiltin("RunAddon(plugin.video.schismAdvanced)")
